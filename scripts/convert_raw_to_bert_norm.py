...
with h5py.File(args.output_path, 'w') as fout:
  for index, line in enumerate(open(args.input_path)):
    ...
    with torch.no_grad():
        ...
        print(encoded_layers[2][0][1].size())
    dset = fout.create_dataset(str(index), (len(tokenized_text), len(tokenized_text)))#FEATURE_COUNT))
    #dset[:,:] = np.vstack([np.array(x) for x in encoded_layers[2][0][1]])
