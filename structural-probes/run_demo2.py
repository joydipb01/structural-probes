...
from BertWithoutAttn import BertWithoutAttn
...
def report_on_stdin(args):
  ...
  model_without_attention = BertWithoutAttn.from_pretrained('bert-base-cased')
  LAYER_COUNT = 24
  FEATURE_COUNT = 1024
  model_without_attention.to(args['device'])
  ...
  for index, line in tqdm(enumerate(sys.stdin), desc='[demoing]'):
    ...
    with torch.no_grad():
      ...
      encoded_layers, _ = model_without_attention(tokens_tensor, segments_tensors)
